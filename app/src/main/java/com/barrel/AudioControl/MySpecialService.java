...
public class MySpecialService extends AccessibilityService
{
    ...
    protected void processAfterGetResult(String Myresult)
    {
        ...
        if (Myresult.equals("开始录像") && RecordFlag == false)
            //这个RecordFlag 用来判断是否为录音状态，来选择是“开始”还是“停止”
        {
            ...
            List<AccessibilityNodeInfo> list = NodeInfoCurrent.findAccessibilityNodeInfosByViewId("com.smartcar.carrecorder:id/video_recoder");
            //根据node 信息和录像按钮控件id 获取录像按钮的准确node位置，存入list 准备点击

            Log.i(TAG, "time for video");
            //log 输出，说明程序已经执行到这一步骤

            if(list.size()!=0)list.get(0).performAction(AccessibilityNodeInfo.ACTION_CLICK);
        ...
        if (Myresult.equals("停止录像") && RecordFlag == true)
            //这个RecordFlag 用来判断是否为录音状态，来选择是“开始”还是“停止”
        {
            CharSequence text = "识别结果是：" +Myresult;
            Toast toast = Toast.makeText(context, text, duration);
        ...
        if (Myresult.equals("拍照"))
        {
            ...
            List<AccessibilityNodeInfo> list = NodeInfoCurrent.findAccessibilityNodeInfosByViewId("com.smartcar.carrecorder:id/photo_capture");
            //根据node 信息和录像按钮控件id 获取录像按钮的准确node位置，存入list 准备点击
    ...
    public void onAccessibilityEvent(AccessibilityEvent event)
    {
        ...
        if (event.getEventType() == AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED && NodeInfoCurrent == null)
            //当AccessibilityService 事件为窗口切换，且当前的NodeInfoCurrent 节点信息记录为空值时，进行第一次初始化
        {
            ComponentName componentName = new ComponentName(event.getPackageName().toString(), event.getClassName().toString());
            Log.i(TAG, "当前Activity为：" + componentName.flattenToShortString());
            //通过AccessibilityService 的方法，读取当前窗口Activity 的名称
            if (componentName.flattenToShortString().equals("com.smartcar.carrecorder/.sc_activity.SCCarRecorderAct"))
                //这个是奇点行车记录仪的工作界面Activity 名称
            {
                NodeInfoCurrent = event.getSource();
                //节点获取为当前事件的触发源，即工作界面的位置

                Log.i(TAG, "赋值成功" + componentName.flattenToShortString());
                //Log输出成功，表明初次赋值成功
            }
        }
